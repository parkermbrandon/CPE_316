...
uint32_t min_val, max_val, avg_val, Frequency;
...
int main(void) {
  ...
  fast_init_f32(&fftHandler, FFT_BUFFER_SIZE);
...
//      if (sample_index >= FFT_BUFFER_SIZE) {
//            // Perform FFT
...
//  if (ADC1->ISR & ADC_ISR_EOC) {
//    if(sample_index < FFT_BUFFER_SIZE) {
//        fftBufIn[sample_index++] = (float)ADC1->DR; // Save ADC value in FFT buffer
//    }
//
//    ADC_ready = 1; // Set the flag
//    ADC1->ISR |= ADC_ISR_EOC; // Clear the EOC flag
